"""Web-sized copies of the staged media, made once and served in their place.

The media a study is staged from is archival: §4's stills are full-resolution
PNGs because the masks were cut from those pixels. That is not what a browser
should be asked to download — a strip of five is 6.4 MB of PNG for a picture
drawn 880 px wide. On campus that is invisible. Over the published link of
``deploy/edge`` it is the wait the participant reads as the instrument being
slow.
...
*A derivative that cannot be made is not an error.* The one entry point falls
back to the source file, once and quietly, and the instrument serves what it
always served. A missing encoder, an unwritable cache or a file Pillow will
...
class Derivatives:
    ...
    def __init__(self, media_dir: Path, *, cache_dir: Path | None = None, enabled: bool = True) -> None:
        ...
        # Sources whose recipe has already failed. A file Pillow will not open
    ...
    def warm(self, stills: Iterable[Path]) -> tuple[int, int]:
        """Make every derivative now, so no participant waits for the first one.

        Returns ``(made, served whole)``. The second is the honest one: it is
        the count of files for which a derivative could not be made, and the
        operator is told rather than left to notice the bandwidth.
        """
        made = 0
        whole = 0
        for source in stills:
            path, _ = self.still(source)
            if path == source:
                whole += 1
            else:
                made += 1
    ...
    def _derive(
        ...
        with self._lock_for(key):
            ...
            try:
                ...
                # last so the name still says what the file is.
