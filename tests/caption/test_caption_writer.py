"""The shared caption machinery: the cache, the budget, and what wrote a caption.

``dpo.caption.writer`` is shared by both instruments and outlives whichever is
archived, so the tests of its cache, its budget enforcement and its provenance
live beside it. What is v1-shaped inside that module — the level rules, the
template's rendering of heads and scene rows, the control-clip instruction —
is tested in tests/session, next to the instrument those distinctions belong
to.
"""
